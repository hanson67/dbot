...
async def 활약도(ctx):
    if checkName(ctx.author.id):
        ...
        embed = discord.Embed(title=f'{ctx.author.name}님의 활약도 : {checkScore(ctx.author.id)}', description='', color=0x4432a8)
        await ctx.send(embed=embed)
...
@bot.command()
async def 활약도추가(ctx, user: discord.User, i):
    if ctx.message.author.guild_permissions.administrator:
        if checkName(user.id):
            await ctx.send("등록되지 않는 사용자입니다.")
        else:
            if int(f'{i}') < -100000 or int(f'{i}') > 100000:
                embed = discord.Embed(title='숫자 범위가 너무 큽니다.', description='', color=0x4432a8)
                await ctx.send(embed=embed)
            else:                
                i = int(f'{i}')
                print(type(ws.cell(findRow(user.id), c_score).value), type(i))
                ws.cell(findRow(user.id), c_score).value += i
                embed = discord.Embed(title=f'{user.name}님의 활약도 : {checkScore(user.id)}', description='', color=0x4432a8)
                await ctx.send(embed=embed)
